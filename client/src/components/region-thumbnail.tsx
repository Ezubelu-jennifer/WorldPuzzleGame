...
interface RegionThumbnailProps {
  ...
  useHardcodedPath?: boolean; // Special flag to use hardcoded paths for problematic regions
}
...
export function RegionThumbnail({
  ...
  useEffect(() => {
    // Always set a consistent viewBox
    setViewBox("0 0 100 100");
    
    // If useHardcodedPath is true, use the hardcoded paths directly for Ebonyi and FCT
    if (useHardcodedPath) {
      console.log(`Using hardcoded path for region: ${regionId}, ${regionName}`);
      if (regionId === "NG-EB" || regionName.includes("Ebonyi")) {
        ...
        setPathData(hardcodedEbonyi);
        return;
      } else if (regionId === "NG-FC" || regionName.includes("Federal Capital Territory") || regionName.includes("FCT")) {
  ...
  }, [svgData, regionId, regionName, useHardcodedPath]);
