...
export function StatePiece({ 
  ...
  const [strongPulseEffect, setStrongPulseEffect] = useState<boolean>(false); // Enhanced visual feedback for correct placement
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => {
    ...
    if (containerRef.current) {
      ...
      // Dynamic threshold that gets smaller as shape size increases (inverse relationship)
      // This makes smaller pieces need to be placed more precisely
      const baseThreshold = 60;
      const dynamicThreshold = baseThreshold * (1 / shapeSize);
      
      // Update nearTarget state with dynamic tolerance
      const isNear = distance <= dynamicThreshold;
      
      // Log for debugging when piece is close
      if (distance < dynamicThreshold * 1.2) {
        console.log(`Piece ${region.name} is close. Distance: ${distance.toFixed(2)}, Threshold: ${dynamicThreshold.toFixed(2)}`);
      }
  ...
  }, [isDragging, containerRef, region.correctX, region.correctY, isNearTarget, shapeSize]);
  ...
  const handleMouseUp = useCallback((e: MouseEvent) => {
    ...
    if (containerRef.current) {
      ...
      if (isDropped) {
        ...
        setStrongPulseEffect(true);
        setTimeout(() => {
          setStrongPulseEffect(false);
          setPulseEffect(true);
          setTimeout(() => {
  ...
  const handleTouchMove = useCallback((e: TouchEvent) => {
    ...
    if (containerRef.current) {
      ...
      // Log for debugging when piece is close
      if (distance < dynamicThreshold * 1.2) {
        console.log(`Touch: Piece ${region.name} is close. Distance: ${distance.toFixed(2)}, Threshold: ${dynamicThreshold.toFixed(2)}`);
      }
          ...
          <>
            ...
            <circle
              ...
              style={{ 
                ...
                filter: !strongPulseEffect && !pulseEffect
